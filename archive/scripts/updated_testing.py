...
def detect_order_blocks(df: pd.DataFrame,
                        ...
                        ob_min_sl_pct: float = 0.001) -> Tuple[List[OrderBlock], List[Signal], List[Position], pd.DataFrame]:
    """
...
end = "2024-02-21"                  # end date for data collection
...
# Detection tunables
swing_length = 30                   # same as Pine's swingLength
break_atr_mult = 0.3         # require breakout > 0.2 * ATR
ob_search_window = swing_length*0.5        # build OB from bars surrounding the selected candle
min_strength_ratio = 0.30    # min ratio (dominant side) to emit a signal

# OB SL floor tunables
ob_min_sl_atr_mult = 1    # ensure SL at least this * ATR
ob_min_sl_pct = 0.001       # ensure SL at least this fraction of entry price

# Position management parameters
stop_loss_multiplier = 1.2         # SL distance = OB_size × this multiplier (SL placed INSIDE the OB)
take_profit_multiplier = 4.5       # TP distance = SL_distance × this multiplier
...
trailing_stop_percent = 2.5        # trail stop by this % below highest (longs) or above lowest (shorts)
...
risk_per_trade_percent = 5.0       # risk this % of capital per trade (used for position sizing)
use_fixed_capital = False           # if True, always use initial capital for position sizing (no compounding)
